    ...
    <div className="bg-white overflow-x-hidden">
        ...
        <div className="relative z-20 h-full flex items-center px-6 sm:px-12 lg:px-20">
          ...
          >
            ...
            <div className="flex flex-wrap gap-4">
              <motion.div whileHover="wiggle" variants={fadeIn}>
                <Link
                  to="/shop"
      ...
      {promos.length > 0 && (
        <div className="max-w-[1400px] mx-auto px-4 lg:px-8 py-16 border-t border-gray-100">
          ...
          <motion.div 
            ...
            viewport={{ once: false, amount: 0.15 }}
            variants={staggerContainer}
            className={`grid gap-4 ${promos.length === 1 ? 'grid-cols-1' : promos.length === 2 ? 'grid-cols-1 md:grid-cols-2' : 'grid-cols-1 md:grid-cols-3'}`}
          >
            {promos.map((promo, idx) => (
              <motion.div key={idx} variants={fadeIn}>
                <Link to="/shop" className="group relative overflow-hidden block font-display">
                  {/* Image */}
      ...
      {gallery.length > 0 && (
        <div className="max-w-[1400px] mx-auto px-4 lg:px-8 py-16 border-t border-gray-100">
          ...
          >
            {gallery.map(img => (
              <motion.div key={img._id} variants={fadeIn}>
                <Link to="/gallery" className="relative group overflow-hidden bg-gray-100 aspect-square block hover:shadow-lg transition-shadow">
                  <img
                    src={img.imageUrl}
